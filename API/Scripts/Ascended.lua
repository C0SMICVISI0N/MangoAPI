...
ENV.VERSION = "v1.1"
ENV.REQUEST = request or http_request or (syn and syn.request) or nil

-- Loading prerequisites
assert(getgenv, "Failed to find exploit environment, please use a better hack!!!")
assert(ENV.REQUEST, "Your exploit requires a HTTP REQUEST function to verify the version you're trying to use this script on is online.")
...
local existingUi = ((gethui and gethui() or services.CoreGUI):FindFirstChild("Ascended V2"))
if ascendOrig then
  hookfunction(Instance.new("RemoteFunction").InvokeServer, ascendOrig.serverPortal)
  ...
  if game:GetService("ReplicatedStorage").LoadSaveRequests:FindFirstChild("AscendBackdoor") then
    game:GetService("ReplicatedStorage").LoadSaveRequests:FindFirstChild("AscendBackdoor"):Destroy()
  end
...
local ascendRemote = Instance.new("RemoteFunction", replicatedStorage.LoadSaveRequests)
ascendRemote.Name = "AscendBackdoor"
...
function ascend:LoadSlot(slot, lsd)
  if lsd then
    ...
    return replicatedStorage.LoadSaveRequests.AscendBackdoor:InvokeServer(slot, { RemoteKey = remoteKey, Password = "DizzlerBackdoor", Command = "NoSaveLoad" })
  else
...
--> anti skiddy
local origNc; origNc = hookfunction(ascendRemote.InvokeServer, function(self, ...)
  if self.Name == "AscendBackdoor" then
    return origNc(replicatedStorage.LoadSaveRequests.RequestLoad, ({...})[1], b)
  end
...
local origNc2; origNc2 = hookmetamethod(game, "__namecall", function(self, ...)
  ...
  if checkcaller() and self.Name == "AscendBackdoor" then
    return origNc2(replicatedStorage.LoadSaveRequests.RequestLoad, ({...})[1], b)
  end
  return origNc2(self, ...)
end)
...
getgenv().ascendOrig = {["connections"] = {}, ["newindex"] = origNIndex, ["namecall"] = origNc2, ["serverPortal"] = origNc }
...
library = (function()
  ...
  function library:Create(name)
    ...
    library.ui.Main.Tabs.Tab_Welcome.CreditsText.Text = library.ui.Main.Tabs.Tab_Welcome.CreditsText.Text .. "\n" .. "Crack By: DalaisonTheBone"
...
local ui = library:Create('Ascended V2')
